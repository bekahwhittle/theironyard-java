...
public class MyGdxGame extends ApplicationAdapter {
	...
	public void render () {
		...
		TextureRegion karmia;
		if (y > 0){
			karmia = jump;
		}
		else if(xv != 0){
			karmia = walk.getKeyFrame(totalTime, true);
		}
		else {
			karmia = stand;
		}
		...
		batch.begin();
		if (faceRight) {
			batch.draw(karmia, x, y, DRAW_WIDTH, DRAW_HEIGHT);
		}
		else {
			batch.draw(karmia, x + DRAW_WIDTH, y, DRAW_WIDTH * -1, DRAW_HEIGHT);
		}
		batch.end();
	}
